...
explorer_server <- function(input, output, session, sclist, useid, cmeta = NULL){
    ...
    output$plot_ui <- renderUI({
        ...
        if(!grepl("3D", input$proj_type, ignore.case = T)) {
            ...
            tagList(
                ...
                # tags$p("Due to limit on computational resources with current server, we excluded expression of some genes in this online version of EHT explorer. You can explore the full version with VisCello.eht R package in this github repo. We apologize for this inconvenience."),
    ...
    observe({
        ...
        if(input$proj_colorBy %in% ev$factor_cols) {
            ...
            } else if(input$proj_colorBy == "Combined_Dataset"){
                factor_color <- combine_dataset_color
            } else {
